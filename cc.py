...
def main():
    ...
    for i in range(N):
        ...
        print(f'in {i}st year, we can get: {round(principal_growth, 2)} + {round(compound_growth, 2)} = {round(sum_growth, 2)}, total={int(total)}, cost={round(cost, 2)}, invest_rate={round(invest_rate, 2)}')
